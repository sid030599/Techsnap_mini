...
from .models import Assignment, College, Files, course, courseTopic, myAssignment, myFiles, mycourses, mytopics, courseUnit, myCourseUnit, Groups, Announcement, grades as marks
...
def topic_detail(request, topicid):
    if request.user.profile.status == 't':
        ...
        return render(request, 'topic_detail.html', {'topic': topic, 'status': 't', 'links': links, 'is_unit': is_unit})
...
def rename_file(request):
    try:
        if request.method == 'POST':
            ...
            os.rename(file.document.path, path + "documents\\" + filename+'_' + documentid + "." + ext)
            file.document.name = "documents/" + filename +'_' + documentid + "." + ext
            file.save()
...
def assignment_detail(request, assignmentid):
    ...
    if request.user.profile.status == 't':
        ...
        return render(request, 'assignment_detail_teacher.html', {'assignment': assignment, 'submissions': submissions, 'links': links})
    links = []
